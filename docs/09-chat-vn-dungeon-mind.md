...
- MVP 的 image mock generation 会创建 generated background `MediaAsset`，回填当前消息首个 `SceneHint.backgroundAssetId`，但不生成真实图片文件；Tauri 预览会显示明确的不可预览降级提示。
- 生成结果必须经过媒体审核字段。
- 不允许把竞品封面作为参考图。
