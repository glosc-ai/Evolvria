...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("cancel_ai_request"):
		...
		return
	if _handle_settings_page_scroll(event):
		get_viewport().set_input_as_handled()
		return
...
func _ensure_input_actions() -> void:
	...
	_register_key_action("cancel_ai_request", [KEY_ESCAPE, KEY_BACKSPACE])
...
func _handle_cancel_or_back() -> void:
	...
	if AppState.current_route == &"main_menu" or AppState.current_route == &"onboarding":
		return
	if AppState.current_route == &"map" and _map_focus_mode:
		_exit_map_focus_mode(true)
		return
...
func _handle_settings_page_scroll(event: InputEvent) -> bool:
	if AppState.current_route != &"settings" or _settings_scroll == null or not is_instance_valid(_settings_scroll):
		return false
	...
	if not key_event.pressed or key_event.echo:
		return false
	var step := maxf(160.0, get_viewport_rect().size.y * 0.82)
	var direction := 0.0
	...
	bar.value = clampf(bar.value + step * direction, bar.min_value, bar.max_value)
	return true
...
func _render_main_menu() -> void:
	...
	var can_continue := SaveManager.has_save() or WorldStore.has_world()
	...
	if not can_continue:
		stack.add_child(_label("暂无可继续的本地世界；请新建世界或从存档列表导入。", 12, MUTED, HORIZONTAL_ALIGNMENT_CENTER))
...
func _render_new_world() -> void:
	...
	var top_bar := _top_bar("新建世界", "返回主菜单", func() -> void: AppState.navigate(&"main_menu"))
	...
	var footer := _new_world_step_footer()
	var footer_height := maxf(76.0 if _wide() else 176.0, footer.get_combined_minimum_size().y)
	footer.set_anchors_preset(Control.PRESET_BOTTOM_WIDE)
	...
	stack.add_child(_new_world_manual_entry_panel())
...
func _new_world_world_step() -> Control:
	...
	_world_name_input = _line_edit_value(str(_new_world_draft.get("world_name", "")).strip_edges(), "输入世界名称")
...
func _new_world_hero_step() -> Control:
	...
	_hero_name_input = _line_edit_value(str(_new_world_draft.get("hero_name", "")).strip_edges(), "输入主角姓名")
	hero_grid.add_child(_field("姓名", _hero_name_input))
	_hero_identity_input = _line_edit_value(str(_new_world_draft.get("hero_identity", "")).strip_edges(), "输入身份")
	hero_grid.add_child(_field("身份", _hero_identity_input))
	_hero_goal_input = _line_edit_value(str(_new_world_draft.get("hero_goal", "")).strip_edges(), "输入目标")
	hero_grid.add_child(_field("目标", _hero_goal_input))
	_hero_ability_input = _line_edit_value(str(_new_world_draft.get("hero_ability", "")).strip_edges(), "输入能力/特质")
	hero_grid.add_child(_field("能力/特质", _hero_ability_input))
	_hero_weakness_input = _line_edit_value(str(_new_world_draft.get("hero_weakness", "")).strip_edges(), "输入弱点")
...
func _new_world_characters_step() -> Control:
	var box := VBoxContainer.new()
	box.add_theme_constant_override("separation", 14)
	box.add_child(_section_title("3. 关键角色", "每行一个角色：姓名 | 身份/定位 | 关系 | 性格关键词 | 秘密/目标 | 行动倾向 | 简述。也支持中文竖线、分号、制表符或空格分隔；可覆盖女主、女二、女三、男二等核心角色。"))
	_key_characters_input = _text_edit(str(_new_world_draft.get("key_characters", "")), 168, "每行输入一个角色。示例：Mira heroine ally calm hidden_goal protects_hero scout")
...
func _new_world_preferences_step() -> Control:
	...
	_limits_input = _text_edit(str(_new_world_draft.get("limits", "")), 96, "输入希望包含或避免的内容")
...
func _new_world_manual_entry_panel() -> Control:
	var row: BoxContainer = HBoxContainer.new() if _wide() else VBoxContainer.new()
	row.add_theme_constant_override("separation", 8)
	var hint := _label("可直接覆盖示例内容；也可以从空白草稿开始。", 12, MUTED)
	hint.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_child(hint)
	row.add_child(_button("清空示例内容", func() -> void:
		_clear_new_world_template()
	, false, INFO))
	return _panel(row, 10, Color(INFO, 0.04), BORDER)
...
func _render_exploration() -> void:
	...
	input_panel.custom_minimum_size = Vector2(0, 108 if _wide() else 118)
	var row: BoxContainer = HBoxContainer.new() if _wide() else VBoxContainer.new()
	row.add_theme_constant_override("separation", 10)
	_action_input = _line_edit("输入你的行动...")
	_action_input.placeholder_text = "输入行动，或先点上方建议"
	...
	, true)
	send.disabled = _busy
	send.tooltip_text = "也可以按 Enter 提交。"
	row.add_child(send)
	...
	input_panel.add_child(_usage_estimate_panel(action_estimate, "发送前预估", false, not _wide()))
	...
	main.add_child(_padded(input_panel, 10 if _wide() else 8))
...
func _render_map() -> void:
	...
	page.add_child(_top_bar(map_title, "返回探索" if not _map_focus_mode else "返回地图", func() -> void:
		if _map_focus_mode:
			_exit_map_focus_mode(true)
		else:
			AppState.navigate(&"exploration")
...
func _render_settings() -> void:
	...
	var stack := _content_stack(760)
	scroll.add_child(stack)
	stack.add_child(_settings_quick_actions())
...
func _settings_quick_actions() -> Control:
	var row: BoxContainer = HBoxContainer.new() if _wide() else VBoxContainer.new()
	row.add_theme_constant_override("separation", 8)
	var hint := _label("PageDown / PageUp 可滚动设置；返回按钮固定在顶部，常用保存操作也放在这里。", 12, MUTED)
	hint.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_child(hint)
	row.add_child(_button("保存设置", func() -> void:
		_save_settings()
	, true))
	row.add_child(_button("返回", func() -> void:
		AppState.navigate(_route_before_settings)
	, false))
...
func _create_world_from_form() -> void:
	...
	if str(seed["world_name"]).is_empty():
		...
		return
	AppState.clear_error()
	_clear_new_world_generation_status(false)
...
func _generate_fantasy_map_from_reference_path(path: String) -> void:
	path = path.strip_edges()
	if path.is_empty():
		AppState.set_error("请先选择参考图文件。")
		_render()
		return
...
func _open_map_file_dialog() -> void:
	...
	_popup_file_dialog(dialog)

func _open_reference_map_file_dialog() -> void:
	var dialog := FileDialog.new()
	dialog.title = "选择参考图生成地图"
	dialog.file_mode = FileDialog.FILE_MODE_OPEN_FILE
	dialog.access = FileDialog.ACCESS_FILESYSTEM
	dialog.filters = PackedStringArray(["*.png, *.jpg, *.jpeg, *.webp ; Reference Images"])
	dialog.size = Vector2i(900, 620)
	dialog.file_selected.connect(func(path: String) -> void:
		dialog.queue_free()
		_generate_fantasy_map_from_reference_path(path)
	)
	_popup_file_dialog(dialog)
...
func _popup_file_dialog(dialog: FileDialog) -> void:
...
func _clear_new_world_template() -> void:
	_new_world_draft = {
		"world_name": "",
		"genre": "自定义",
		"tone": "冒险",
	...
	_clear_new_world_generation_status()
	_render()

func _clear_new_world_generation_status(clear_last_seed: bool = true) -> void:
	if _last_failed_operation == &"world_expand":
		_last_failed_operation = &""
	if clear_last_seed:
		_last_world_seed = {}
	if _active_operation == &"world_expand" and not _busy:
		_active_operation = &""
	AppState.clear_error()
...
func _new_world_preview_panel(seed: Dictionary, estimate: Dictionary) -> Control:
	...
	var ability := str(hero.get("ability", "")).strip_edges()
	var weakness := str(hero.get("weakness", "")).strip_edges()
	box.add_child(_preview_row("主角能力/弱点", "%s / %s" % [
		ability if not ability.is_empty() else "未填写",
		weakness if not weakness.is_empty() else "未填写"
	]))
	var limits := str(seed.get("limits", "")).strip_edges()
	box.add_child(_preview_row("内容偏好/禁用", limits if not limits.is_empty() else "未填写"))
	...
	if not parsed_characters.is_empty():
		...
		for index in range(limit):
			box.add_child(_key_character_preview_block(parsed_characters[index] as Dictionary, index + 1))
...
func _key_character_preview_block(character: Dictionary, index: int) -> Control:
	var block := VBoxContainer.new()
	block.add_theme_constant_override("separation", 4)
	block.add_child(_label("%d. %s" % [index, str(character.get("name", "未命名")).strip_edges()], 13, FG, HORIZONTAL_ALIGNMENT_LEFT, true))
	block.add_child(_label("身份：%s · 关系：%s" % [
	...
	if not personality.is_empty():
		block.add_child(_label("性格：%s" % personality, 12, MUTED))
	if not goal.is_empty():
		block.add_child(_label("目标/秘密：%s" % goal, 12, MUTED))
	if not action.is_empty():
		block.add_child(_label("行动倾向：%s" % action, 12, MUTED))
	if not description.is_empty():
		block.add_child(_label("简述：%s" % description, 12, MUTED))
	return block
...
func _fill_key_character_preview(preview: VBoxContainer, text: String, max_rows: int = 0) -> void:
	...
	for index in range(limit):
		preview.add_child(_key_character_preview_block(characters[index], index + 1))
...
func _map_sidebar() -> Control:
	...
	sidebar.add_child(_section_title("地图图片", "选择本机 PNG/JPG/WebP 作为底图或参考图；导出 ZIP 时会一并打包。"))
	...
	sidebar.add_child(_button("从参考图生成地图", func() -> void:
		_open_reference_map_file_dialog()
	, false, INFO))
...
func _map_focus_panel() -> Control:
	...
	var title := "地图主视图"
	var hint := "Esc 或下方按钮返回地图；F 可切换，滚轮/双指缩放，方向键/WASD 平移。"
	...
	controls.add_child(_button("退出主视图", func() -> void:
		_exit_map_focus_mode(true)
	, true))
	controls.add_child(_button("返回探索", func() -> void:
		_exit_map_focus_mode(false)
		AppState.navigate(&"exploration")
	, false))
...
func _on_map_view_gui_input(event: InputEvent, map_view: Control) -> void:
	var handled := false
	if event is InputEventMouseButton:
		handled = _handle_map_mouse_button(event as InputEventMouseButton)
	elif event is InputEventMouseMotion:
		handled = _handle_map_mouse_motion(event as InputEventMouseMotion)
	elif event is InputEventScreenTouch:
		handled = _handle_map_touch(event as InputEventScreenTouch)
	elif event is InputEventScreenDrag:
		handled = _handle_map_drag(event as InputEventScreenDrag)
	elif event is InputEventMagnifyGesture:
		handled = _handle_map_magnify_gesture(event as InputEventMagnifyGesture)
	if handled:
		map_view.accept_event()

func _handle_map_mouse_button(event: InputEventMouseButton) -> bool:
	if event.button_index == MOUSE_BUTTON_WHEEL_UP and event.pressed:
		_zoom_map_around(event.position, _map_zoom * 1.12)
		return true
...
func _handle_map_mouse_motion(event: InputEventMouseMotion) -> bool:
...
func _handle_map_touch(event: InputEventScreenTouch) -> bool:
	if event.pressed:
		...
		if _touch_points.is_empty():
			...
			var remaining := _touch_points.values()[0] as Vector2
			_map_dragging = true
			_map_drag_last = remaining
			_map_drag_moved = true
			_last_pinch_distance = 0.0
	return true

func _handle_map_drag(event: InputEventScreenDrag) -> bool:
	_touch_points[event.index] = event.position
...
func _handle_map_magnify_gesture(event: InputEventMagnifyGesture) -> bool:
	var factor := clampf(event.factor, 0.75, 1.35)
	if is_equal_approx(factor, 1.0):
		return false
	_zoom_map_around(event.position, _map_zoom * factor)
	return true
...
func _exit_map_focus_mode(show_notice: bool = false) -> void:
	if not _map_focus_mode:
		return
	_map_focus_mode = false
	_map_pan = _clamped_map_pan()
	if show_notice:
		AppState.set_notice("已返回地图。")
	_render()
...
func _map_focus_view_size() -> Vector2:
	...
	var width := maxf(300.0, viewport_size.x - 64.0)
	var reserved_height := 360.0 if _wide() else 460.0
	var height := maxf(360.0, viewport_size.y - reserved_height)
...
func _usage_estimate_panel(estimate: Dictionary, title: String = "AI 消耗预估", wrap_panel: bool = true, compact: bool = false) -> Control:
	...
	if compact:
		box.add_child(_label(AIService.estimate_usage_text(estimate), 11, MUTED))
		return box
...
func _line_edit(placeholder: String) -> LineEdit:
	...
	input.add_theme_color_override("font_selected_color", SURFACE)
	input.add_theme_color_override("selection_color", Color(ACCENT, 0.45))
	input.add_theme_color_override("caret_color", ACCENT)
	return input

func _line_edit_value(text: String, placeholder: String) -> LineEdit:
	var input := _line_edit(placeholder)
	input.text = text
	input.placeholder_text = placeholder
	return input

func _text_edit(text: String, height: int, placeholder: String = "") -> TextEdit:
