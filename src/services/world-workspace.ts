...
function buildMapMarkdown(payload: SavePayload): string {
  ...
  return [
    ...
    `- 生成规则：创世时生成并锁定，后续不可新增地点、地区或路线。`,
    "",
    "## 地区",
    "",
    ...(world.map_regions ?? []).map((region) => `- ${region.name}（${region.type}）：${region.description}`),
    "",
    "## 地点索引",
    "",
...
function buildCharacterMarkdown(character: Character, payload: SavePayload): string {
  ...
  return [
    ...
    "## 外貌与形象",
    "",
    character.appearance_description || "暂无。",
    "",
    `- 形象提示词：${character.portrait_prompt ?? "未生成"}`,
    `- 形象更新时间：${character.portrait_updated_at ?? "未生成"}`,
    "",
    "## 性格与目标",
    "",
