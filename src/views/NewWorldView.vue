    ...
    <div class="mt-5 grid gap-5 lg:grid-cols-[1fr_340px]">
      <div class="e-panel p-5">
        ...
        <div v-else-if="step === 2" class="space-y-4">
          ...
          <label class="block text-sm">能力/弱点<input v-model="draft.hero.ability" class="e-field mt-2" /><input v-model="draft.hero.weakness" class="e-field mt-2" /></label>
        </div>
        <div v-else-if="step === 3" class="space-y-4">
          <div v-for="(character, index) in draft.key_characters" :key="index" class="rounded-md border border-white/10 bg-black/20 p-3">
            <div class="grid gap-3 sm:grid-cols-2">
              <input v-model="character.name" class="e-field" placeholder="姓名" />
              <input v-model="character.role" class="e-field" placeholder="身份" />
              <input v-model="character.relationship" class="e-field" placeholder="与主角关系" />
              <input v-model="character.personality" class="e-field" placeholder="性格标签" />
              <input v-model="character.goal" class="e-field" placeholder="目标" />
              <input v-model="character.secret" class="e-field" placeholder="秘密" />
            </div>
            <textarea v-model="character.action_tendency" class="e-field mt-3 min-h-20" placeholder="行动倾向" />
          </div>
        ...
        <div class="mt-6 flex justify-between">
          ...
          <button v-else class="e-btn e-btn-primary" :disabled="world.busy" type="button" @click="create">{{ world.busy ? "正在扩写..." : "创建并扩写世界" }}</button>
        </div>
      </div>
