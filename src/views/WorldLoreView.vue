      ...
      <Card class="p-5">
        ...
        <div class="mt-3 grid gap-3 md:grid-cols-2">
          <div v-for="faction in world.factions" :key="faction.id" class="rounded-md bg-black/24 p-3 text-sm">
            ...
            <div class="text-muted-foreground mt-1">{{ faction.agenda }}</div>
          </div>
        </div>
      </Card>
    </div>
    <aside class="space-y-5">
      <Card class="p-5">
        ...
        <div class="mt-3 grid grid-cols-3 gap-2 text-center text-sm">
          <div class="rounded bg-black/24 p-3"><div class="text-muted-foreground text-xs">调用</div><div class="mt-1">{{ world.usageSummary.calls }}</div></div>
          <div class="rounded bg-black/24 p-3"><div class="text-muted-foreground text-xs">成功</div><div class="mt-1">{{ world.usageSummary.success_count }}</div></div>
          <div class="rounded bg-black/24 p-3"><div class="text-muted-foreground text-xs">Token</div><div class="mt-1">{{ world.usageSummary.total_tokens }}</div></div>
        </div>
      </Card>
      <Card class="p-5">
        <h2 class="font-medium">一致性检查</h2>
        <p class="text-muted-foreground mt-2 text-sm">{{ world.consistencyIssues.length ? `发现 ${world.consistencyIssues.length} 个问题` : "当前未发现断裂引用。" }}</p>
